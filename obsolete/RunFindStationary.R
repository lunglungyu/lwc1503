...
for (i in c(yearBegin:yearEnd)){
	...
	for (j in c(1:n)){
		...
		if(file.exists(SrcfileName)){
			...
			for (mm in c(1:12)){
				...
				llll <- subset(d1,(YEARMODA >= as.Date(begind,"%Y-%m-%d")) & (YEARMODA <= as.Date(endd,"%Y-%m-%d")));
				dfneed <- ndiffs(llll$TEMP,alpha=0.05,test=c("kpss"));
				if(dfneed > mx)mx <- dfneed;
				dfneed2 <- ndiffs(llll$TEMP,alpha=0.05,test=c("adf"));
				if(dfneed2 > mx)mx <- dfneed2;
				dfneed3 <- ndiffs(llll$TEMP,alpha=0.05,test=c("pp"));
				if(dfneed3 > mx)mx <- dfneed3;
				print(sprintf("%s %d %d dfneed:%d %d %d",as.character(sc[j,1]),i,mm,dfneed,dfneed2,dfneed2));
	...
	print(sprintf("Least phase to make staionary:%d",mx));
}
