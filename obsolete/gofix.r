...
for (i in c(yearBegin:yearEnd)){
	...
	for (j in c(60:n)){
		...
		if(file.exists(SrcfileName)){
			...
			#as.numeric(levels(d1$TEMP))[d1$TEMP]
			d1 <- merge(AllDateInYear,d1,all.x=TRUE,na.rm=FALSE);
			d1$TEMP <- na.approx(d1$TEMP,na.rm=FALSE);
			d1$TEMP <- replace(d1$TEMP, is.na(d1$TEMP), median(d1$TEMP, na.rm=TRUE));
			for(k in c(1:n)){
				# print(k);
				if(j!=k){
					...
					if(file.exists(DstfileName)){
						...
						#as.numeric(levels(d2$TEMP))[d2$TEMP]
						...
						#write.table(t(thisHead), file=thisOutputFileName, append=T, row.names=F, col.names=F,  sep=",");
								d2$TEMP <- na.approx(d2$TEMP,na.rm=FALSE);
								d2$TEMP <- replace(d2$TEMP, is.na(d2$TEMP), median(d2$TEMP, na.rm=TRUE));
								#print(d1);
								#print(d2);
						for (mm in c(1:12)){
							...
							if(nrow(d1cut)==nrow(d2cut) & nrow(d1cut)>0){
								...
								stationaryTS1 <- d1cut$TEMP;
								stationaryTS2 <- d2cut$TEMP;
