...
def haversine( lat1, lng1, lat2, lng2 ):
    ...
    lng1, lat1, lng2, lat2 = map(toRadian, [lng1, lat1, lng2, lat2])
    # haversine formula 
    dlon = lng2 - lng1 
    dlat = lat2 - lat1 
...
n_arrows = 0
with open('generalDirections_lag5.txt','w') as f:
			...
			for mmdd in mmdds:
				...
				if mmdd in gcFromTo[yy][s1]:
					for s2 in gcFromTo[yy][s1][mmdd]:
						#print gcFromTo[yy][s1][mmdd][s2][4]
						if float(gcFromTo[yy][s1][mmdd][s2][4]) < 0.05:
							single_gc = {}
							single_gc['s1'] = s1
							single_gc['s2'] = s2
				...
				if mmdd in gcToFrom[yy][s1]:
					for s2 in gcToFrom[yy][s1][mmdd]:
						if float(gcToFrom[yy][s1][mmdd][s2][4]) < 0.05:
							single_gc = {}
							single_gc['s1'] = s1
							single_gc['s2'] = s2
