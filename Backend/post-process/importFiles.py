# generate a file with 76 stations data (each day temperature)
# from ChinaAll (deleted)
...
stationList = []	# 76 stations
station = {}		# station[s][y][m][d] = temperature
...
# 76 stations
with open(os.path.join('..','fypCutStation')) as f:
	for line in f:
		line = line.rstrip()
...
for filename in os.listdir(os.path.join('..','ChinaAll')):
	if len(filename) > 18:
...
missingList = []
raw_file_name = '76raw.txt'
with open(raw_file_name,'w') as raw_out:
	for s in station:
		for yy in station[s]:
			...
			for mm in station[s][yy]:
				for dd in station[s][yy][mm]:
					...
					if 'temp' in station[s][yy][mm][dd]:
						temp = station[s][yy][mm][dd]['temp']
					else:
						temp = None
					tmp = [s,yy,mm,dd,temp]

					tmp = [str(q) for q in tmp]
					raw_out.write(','.join(tmp))
					raw_out.write('\n')
