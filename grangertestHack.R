...
grangertest.default <- function(x, y, order = 1, na.action = na.omit, ...)
{
  ...
  #print(sprintf("xsize:%d",str(x)));
  #print(sprintf("ysize:%d",str(y)));
  #print(sprintf("lagXsize:%d",str(lagX)));
  #print(sprintf("lagYsize:%d",str(lagY)));
  #print(sprintf("allsize:%d",str(all)));
  ...
  #print(str(rval));
  #print(rval[2,4]);
  #print(rval[which(colnames(rval)=="Pr(>F)")][2]);
  #str(rval$'Pr(>F)');
  #return(rval)
  return (rval[2,4]);
}
